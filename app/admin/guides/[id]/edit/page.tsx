...
import { updateGuide } from "@/lib/api/guides";
import { motion } from "framer-motion";
import { ArrowLeft, Save, Plus, X, Loader2, MapPin } from "lucide-react";
import { PageSkeleton } from "@/components/ui/Skeletons";
import ErrorState from "@/components/ui/ErrorState";
import type { Guide, RegistryItem } from "@/lib/schemas";
...
function EditGuidePageInner() {
  ...
  useEffect(() => {
    getRegistryItems({ status: "published" }).then((r) => setRegistryItems(r.items)).catch(() => {});
  }, []);
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      setLoading(true);
      try {
        const guide = await getGuideById(id as string);
        if (!guide) {
  ...
  const handleSave = async () => {
    ...
    setSaving(true);
    try {
      await updateGuide(id as string, {
        title: form.title,
        content: form.content,
        excerpt: form.excerpt,
        registrySlug: form.registrySlug,
        hyperlocalTags: form.hyperlocalTags || [],
        mediaUrls: form.mediaUrls || [],
        status: form.status || "published",
      });
      toast("Guide updated!", "success");
      router.push("/admin");
    } catch (err: any) {
  ...
  if (error) return <ErrorState title="Guide Not Found" message={error} />;
    ...
    <div className="flex flex-col min-h-screen">
      ...
      <main className="grow max-w-4xl mx-auto w-full px-4 sm:px-6 lg:px-8 py-12">
        ...
        <header className="mb-12">
          <h1 className="text-4xl font-bold tracking-tighter uppercase font-sans">Edit Guide</h1>
          <p className="text-slate-500 font-mono text-sm mt-1">
            {form.authorName && <>by {form.authorName}</>}
          </p>
        </header>
